...
function App() {
  ...
  const ensureTodayQuote = useCallback(
    (quotes: Quote[]) => {
      ...
      setTodayQuote((current) => {
        if (!current) {
          return withBackground(quotes[Math.floor(Math.random() * quotes.length)])
        }
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        setIsLoading(true)
        const storedCustom = typeof window !== 'undefined' ? safeParseQuotes(window.localStorage.getItem(CUSTOM_STORAGE_KEY)) : []
        const storedPinned = typeof window !== 'undefined' ? safeParseQuotes(window.localStorage.getItem(PINNED_STORAGE_KEY)) : []

        const normalizedCustom = storedCustom.map((quote) => ({ ...quote, isCustom: true }))
        ...
        const mergedQuotes = uniqueById([...normalizedBase, ...normalizedCustom])
        const normalizedPinned = storedPinned.map((quote) => withBackground(quote))
        ...
        console.error(err)
        if (!cancelled) {
          setError('名言の読み込みに失敗しました。リロードして再試行してください。')
        }
      } finally {
        if (!cancelled) {
          setIsLoading(false)
  ...
  const handleShuffleQuote = () => {
    ...
    setTodayQuote((previous) => {
      ...
      let next: Quote
      do {
        next = withBackground(allQuotes[Math.floor(Math.random() * allQuotes.length)])
  ...
  const handleUpdatePinned = (id: string, text: string, author: string) => {
    ...
    setPinnedQuotes((prev) => {
      const next = prev.map((quote) => (quote.id === id ? { ...quote, text: trimmedText, author: trimmedAuthor } : quote))
      persistPinned(next)
      return next
    })
  ...
  return (
    <div className="app">
      ...
      <nav className="app__nav">
        <button
          type="button"
          className={`nav-button ${view === 'today' ? 'nav-button--active' : ''}`}
        ...
        <button
          type="button"
          className={`nav-button ${view === 'favorites' ? 'nav-button--active' : ''}`}
...
function TodayQuoteSection({ isLoading, quote, isPinned, onPin, onShuffle }: TodayQuoteSectionProps) {
  if (isLoading) {
    return <div className="empty-state">名言を読み込んでいます...</div>
  }

  if (!quote) {
    return <div className="empty-state">表示できる名言がありません。名言を登録してみましょう。</div>
  }

  return (
    <section className="today-card" aria-live="polite">
      <div className="today-card__bg" style={{ backgroundImage: `url(${quote.background ?? getRandomBackground()})` }} />
      ...
      <div className="today-card__actions">
        ...
        <button type="button" className="button button--ghost" onClick={onShuffle}>
          <ShuffleIcon />
          別の格言
        </button>
      </div>
...
function FavoritesSection({ quotes, onDelete, onUpdate }: FavoritesSectionProps) {
  ...
  if (!quotes.length) {
    return <div className="empty-state">ピン留めした名言がここに表示されます。</div>
  }
            ...
            <article key={quote.id} className="favorite-card" style={style}>
              {isEditing ? (
                <form onSubmit={submitEdit}>
                  <textarea value={draftText} onChange={(event) => setDraftText(event.target.value)} required />
                  <input
                    type="text"
                    value={draftAuthor}
                ...
                <>
                  ...
                  <div className="favorite-card__actions">
                    <button type="button" className="button button--ghost" onClick={() => startEdit(quote)}>
                      <EditIcon />編集
                    </button>
                    <button type="button" className="button button--ghost" onClick={() => onDelete(quote.id)}>
                      <TrashIcon />削除
                    </button>
                  </div>
                </>
  ...
  return (
    <section>
      ...
      <form className="add-form" onSubmit={handleSubmit}>
        ...
        {error && <span className="helper-text" role="alert">{error}</span>}
